...
for areatree in areatrees: 
    ...
    for node in root.getSubtreeNodes():
        ...
        if tags is None:
            continue
        for tag in tags:
            foundtags.append( ['areatrea', areatree.getId(), node.computeDegree(), node.getLayer().getId(), areatree.getTitle(), areatree.getProject().getShortMeaningfulTitle(areatree), str(node), str(tag)] )
...
for connector in connectors:
    ...
    for node in root.getSubtreeNodes():
        ...
        if tags is None:
            continue
        for tag in tags:
            foundtags.append( ['connector', connector.getId(), node.computeDegree(), node.getLayer().getId(), connector.getTitle(), connector.getProject().getShortMeaningfulTitle(connector), str(node), str(tag)] )
